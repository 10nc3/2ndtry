...
// FIX #15: Hoist data-package require out of toDataPackage() so the
// dependency is visible at module load time and failures surface early.
...
// FIX #32: PhiMemory construction is sync but bootstrap() is async. Callers
// must `await state.bootstrapMemory()` once at session start, otherwise
// getMemoryContext() returns an empty window even when the ledger has data.
// We additionally lazy-bootstrap on the first getMemoryContext() call so a
// forgetful caller still gets correct behaviour (at the cost of one extra
// await on first use).
class PipelineState {
   ...
   * FIX #32: Lazy-bootstrap if the caller forgot. Returns sync result if
   * already bootstrapped (preserves the original sync API), otherwise
   * returns a Promise that resolves to the same shape.
   */
  getMemoryContext() {
    if (!this.phiMemory.bootstrapped) {
      return this.phiMemory.bootstrap().then(() => this.phiMemory.getContext());
    }
  ...
  setError(error, recoverable = false) {
    // FIX #9: Previously read `.message` / `.stack` off whatever was passed
    // in, which crashed when the caller threw a string or a plain object.
    // Coerce to a real Error so downstream serializers always see a stack.
    const e = error instanceof Error ? error : new Error(
      typeof error === 'string' ? error : JSON.stringify(error)
    );
    this.error = { message: e.message, stack: e.stack, recoverable };
    this.step = 'ERROR';
  }
