 ...
 *
 * 📚 MIND PALACE: see AGENTS.md § "Memory Architecture"
 *   Books are storage, not just logs. Photos, documents, audio,
 *   video, code snapshots, links — anything I generate.
 *   Four rooms: Book 1 (journal), Book 2 (auto-sync), Book 3
 *   (owner-spare), Book 4 (my archive when context limits bite).
 *
 * Tokens: NYAN_PLAYGROUND_TOKEN, NYAN_BOOK1-4_TOKEN
 */
...
const BOOK_3 = {
  id: 'dev_book_t1_084666a81896',
  name: 'Avijja Spare',
  endpoint: 'https://nyanbook.io/api/webhook/dev_book_t1_084666a81896',
  get token() { return getCred('NYAN_BOOK3_TOKEN'); },
};

const BOOK_4 = {
  id: 'dev_book_t1_48348d84129e',
  name: 'Avijja Reserve',
  endpoint: 'https://nyanbook.io/api/webhook/dev_book_t1_48348d84129e',
  get token() { return getCred('NYAN_BOOK4_TOKEN'); },
};
...
function health() {
  return {
    ...
    book3: {
      name: BOOK_3.name,
      endpoint: BOOK_3.endpoint,
      token: maskToken(getToken('NYAN_BOOK3_TOKEN')),
      ready: !!getToken('NYAN_BOOK3_TOKEN'),
    },
    book4: {
      name: BOOK_4.name,
      endpoint: BOOK_4.endpoint,
      token: maskToken(getToken('NYAN_BOOK4_TOKEN')),
      ready: !!getToken('NYAN_BOOK4_TOKEN'),
    },
    webhook: {
...
function guard(required = ['playground', 'book1', 'book2', 'book3']) {
  const h = health();
  const missing = [];
  ...
  if (required.includes('book3') && !h.book3.ready) missing.push('NYAN_BOOK3_TOKEN');
  if (required.includes('book4') && !h.book4.ready) missing.push('NYAN_BOOK4_TOKEN');
