...
const { BOOK_1, BOOK_2, BOOK_3, auth, health, guard } = require('./config');
...
async function ledgerAppend(entry, target = 'book_2') {
  ...
  return {
    ...
    status: res.status,
    target,
    bookName: target === 'book_1' ? BOOK_1.name : target === 'book_3' ? BOOK_3.name : BOOK_2.name,
  };
}
...
async function ledgerHealth() {
  ...
  return {
    ...
    book2: book2Read,
    book3: book3Read,
    ready: h.book1.ready && h.book2.ready && h.book3.ready && book1Read?.ok && book2Read?.ok && book3Read?.ok,
  };
}
