...
import { initSidebarCat } from "./cat-animation.ts";
import { initSidebarClock } from "./sidebar-clock.ts";
import {
...
export class OpenClawApp extends LitElement {
  ...
  private _catCleanup: (() => void) | null = null;
  private _clockCleanup: (() => void) | null = null;
  private _clockSync: (() => void) | null = null;
  ...
  private _initCat() {
    const canvas = this.querySelector("#sidebarCat") as HTMLCanvasElement | null;
    if (canvas) {
      this._catCleanup?.();
      this._catCleanup = initSidebarCat(canvas);
    }
  }

  private _initClock() {
    const digitalEl = this.querySelector("#sidebarClockDigital") as HTMLElement | null;
    const canvas = this.querySelector("#sidebarClockAnalog") as HTMLCanvasElement | null;
    if (digitalEl && canvas) {
      this._clockCleanup?.();
      const clock = initSidebarClock(digitalEl, canvas, () => this.settings.navCollapsed);
      this._clockCleanup = clock.cleanup;
      this._clockSync = clock.sync;
    }
  }
  ...
  protected override updated(changed: Map<PropertyKey, unknown>) {
    ...
    // Re-init cat if canvas was (re-)created
    requestAnimationFrame(() => this._initCat());
    // Sync clock mode on nav collapse/expand
    if (changed.has("settings")) {
      requestAnimationFrame(() => this._clockSync?.());
    }
